...
export default function ChunkUploader() {
	...
	const hideCreateFolder = () => {
		gsap.to(folderRef.current, {
			...
			duration: 1.5,
			ease: 'power1.inOut',
			onComplete: () => revealUploader(),
		})
	}

	const revealUploader = () => {
	...
	return (
		<div className='max-w-4xl mx-auto p-4'>
			...
			/>
			{folderId && (
				<div className='mt-6 opacity-0' ref={uploadRef}>
					<div
						{...getRootProps()}
						className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${
							isDragActive
								? 'border-blue-400 bg-blue-50'
								: uploading // Add uploading check
								? 'border-gray-300 bg-gray-100 cursor-not-allowed' // Disabled state
					...
					<div className='mt-4 flex justify-end gap-4'>
						{files.length > 0 && (
							<button
								...
								className='px-4 py-2 text-gray-600 hover:text-gray-800 opacity-0'
								ref={resetRef}
							>
								Upload More Files
							</button>
